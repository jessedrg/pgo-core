...
describe("AccountPaymentMethod", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: AccountPaymentMethodService,
          useValue: service,
        },
      ],
  ...
  test("POST /account-payment-methods", async () => {
  ...
  test("GET /account-payment-methods/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/account-payment-methods"}/${nonExistingId}`)
      .expect(404)
      .expect({
        statusCode: 404,
  ...
  test("GET /account-payment-methods/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/account-payment-methods"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
